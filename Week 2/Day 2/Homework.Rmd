...
_These remaining 493 tweets have been favorited a total of 425 times_
...
tweets %>%
  ...
  group_by(media_type) %>% 
  summarise(number_of_likes = mean(favorite_count)) %>% 
...
_if all data is unique this should provide 27 + 15 variables (-1 for join variable) = 41
```{r}
...
code_clan_hashtags %>% 
  ...
  mutate(hashtags_lower = str_detect(hashtags, "edinburgh")) %>% 
  filter(hashtags_lower == TRUE)
```
#Question 10.
...
#this shows the frequeny at which someone is specifically tweeted
tweets %>% 
  select(text) %>% 
