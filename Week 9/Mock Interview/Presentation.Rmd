...
by_year %>% 
  filter(grepl("skin", cancer_site),
         sex == "Male") %>% 
  ggplot(aes(x = year, y = incidences_all_ages, colour = cancer_site), ymax = 220)+
  geom_line()+
  geom_smooth(method = "lm", se = FALSE)+
...
by_year %>% 
  ...
  geom_line()+
  geom_smooth(method = "lm", se = FALSE)+
      labs(title = "Incidences of least suvivable cancers from 1996 to 2020",
       x = "Year",
       y = "Number of Incidences")
...
by_age %>% 
  ...
  select(-incidences_age5to9) %>% 
  pivot_longer(cols = c(incidences_age_under5:incidences_age85and_over, incidences_age05to09), names_to = "age_range", values_to = "cases") %>% 
  filter(cancer_site == "Trachea, bronchus and lung", sex == "All") %>% 
  distinct(age_range, cases) %>%
  arrange(age_range)
...
by_age %>% 
  ...
  arrange(age_range) %>% 
    ggplot()+
  geom_col(aes(y = age_range, x = cases), fill = "coral")+
  labs(title = "Total incidences of by age",
       x = "Number of Incidences",
       y = "Age range")
...
by_age %>% 
  ...
  arrange(age_range) %>% 
    ggplot()+
  geom_col(aes(y = age_range, x = cases), fill = "blue")+
    labs(title = "Rate of incidences by age",
       x = "Number of Incidences per 100,000",
       y = "Age range")
```
